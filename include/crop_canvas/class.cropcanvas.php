...
/*	if (ereg("[a-zA-Z0-9._-]+\.jpg", $file)) $ok = imagejpeg($imatge, $file, 100);
        else if (ereg("[a-zA-Z0-9._-]+\.png", $file)) $ok = imagepng($imatge, $file);
        else $ok = imagegif($imatge, $file);*/

        $handle = fopen($file, "r");
        $content = fread($handle, $file_size);
        fclose($handle);
//        $this->image_sizes = getimagesize($file);
        return $content;
    }

    function loadStringFromImage($id)
    {
        $file = "/tmp/$id.jpg";
        $ok = imagejpeg($this->_imgFinal, $file, 100);
        $file_size = filesize($file);
        $handle = fopen($file, "r");
        $content = fread($handle, $file_size);
        fclose($handle);
    ...
    function resizeImageFromImage($image, $newwidth, $newheight)
    {
        //$this->_imgFinal = imagecreatetruecolor($newwidth, $newheight);

	$this->_imgFinal = $image;
	$string = $this->loadStringFromImage(rand(1,999999));
	$this->_imgFinal = $this->imagecopyresized($string, $newwidth, $newheight);

        //imagecopyresized($this->_imgFinal, $image, 0, 0, 0, 0, $newwidth, $newheight, imagesx($image), imagesy($image));
        //imageantialias($this->_imgFinal, true);
    }

    function resizeImage($image_string, $width, $height, $newwidth, $newheight)
    {
        //$this->_imgFinal = imagecreatetruecolor($newwidth, $newheight);
        //$image = imagecreatefromstring($image_string);
        //imagecopyresized($this->_imgFinal, $image, 0, 0, 0, 0, $newwidth, $newheight, $width, $height);
        //imageantialias($this->_imgFinal, true);

        $this->_imgFinal = $this->imagecopyresized($image_string, $newwidth, $newheight);
    }

    function combineImage($image_string, $image_width, $image_height, $x_ini, $y_ini, $x_fi, $y_fi, $opacity=100)
    {
        $newheight = abs($y_fi - $y_ini);
        $newwidth = abs($x_fi - $x_ini);
        $resized_image = imagecreatetruecolor($newwidth, $newheight);


//	$resized_image = $this->imagecopyresized($image_string, $newwidth, $newheight);

        $image = imagecreatefromstring($image_string);
        imagecopyresized($resized_image, $image, 0, 0, 0, 0, $newwidth, $newheight, $image_width, $image_height);


        imagecopymerge($this->_imgOrig, $resized_image, $x_ini, $y_ini, 0, 0, $newwidth, $newheight, $opacity);
        $this->_imgFinal = $this->_imgOrig;
    }

    function getImageWidth()
    {
        return imagesx($this->_imgFinal);
    }

    function getImageHeight()
    {
        return imagesy($this->_imgFinal);
    }
     ...
     * @access public
     */
    function showImage($type = 'jpg', $quality = 90, $name='')
    {
        if ($this->_imgFinal == null) {
    ...
    /**
     * Function to deliver an image to a wireless device
     *
     * @param  string $type
     * @param  int    $quality
     * @return bool
     * @access public
     */
    function deliverImage($type = 'jpg', $quality = 90, $name='')
    {
        if ($this->_imgFinal == null) die(ERROR_TECNICAL_PROBLEM);

        $type = strtolower($type);
        $func = 'image' . ($type == 'jpg' ? 'jpeg' : $type);
        $head = 'image/' . ($type == 'jpg' ? 'jpeg' : $type);

        if (!$this->_isSupported('[showing file]', $type, $func, false)) die(ERROR_TECNICAL_PROBLEM);
        ...
        switch($type)
	{
            ...
            case 'jpg':
                $func($this->_imgFinal, '', $quality);
                break;
	    default: die(ERROR_TECNICAL_PROBLEM); break;
        }
    ...
    function adjustDimensionsToMinimSize($min)
    {
        $width = $this->width;
        $height = $this->height;
        ...
        {
                $newwidth = $min;
                $newheight = ($height * $min)/$width;
        }
        else if(($width>$height)&&($height!=$min))
        {
                $newheight = $min;
                $newwidth = ($width * $min)/$height;
        }
        ...
        $this->width = $newwidth;
        $this->_imgFinal = imagecreatetruecolor($newwidth, $newheight);
        return imagecopyresized($this->_imgFinal, $this->_imgOrig, 0, 0, 0, 0, $newwidth, $newheight, $width, $height);
    }

    function adjustDimensionsToRange($string, $min_px, $max_px)
    {
        $width = $this->width;
        $height = $this->height;
        ...
        {
                $newwidth = $max_px;
                $newheight = ($height * $max_px)/$width;
        }
        else if(($width<$height)&&($height>$max_px))
        {
                $newheight = $max_px;
                $newwidth = ($width * $max_px)/$height;
        }
        else if(($width<$height)&&($width<$min_px))
        {
                $newwidth = $min_px;
                $newheight = ($height * $min_px)/$width;
        }
        else if(($width>$height)&&($height<$min_px))
        {
                $newheight = $min_px;
                $newwidth = ($width * $min_px)/$height;
        }
        else
        {
                $this->height = $height;
...
//        return imagecopyresized($this->_imgFinal, $this->_imgOrig, 0, 0, 0, 0, $newwidth, $newheight, $width, $height);
	...
	function imagecopyresized($string, $newwidth, $newheight, $extensio='jpg')
	{
		$file = "/var/www/html/tmp/".(rand(1,999999));
//	        $ok = imagejpeg($image, $file.".$extensio", 100);
		$fh=fopen($file.".$extensio","w");
		fwrite($fh,$string);
		fclose($fh);

		system("/usr/bin/convert $file.$extensio -antialias -quality 100 -resize $newwidth"."x$newheight! $file"."_resized.$extensio");

//		print "/usr/bin/convert $file.jpg -resize $newwidth"."x$newheight $file"."_resized.jpg";
		$image_string = file_get_contents($file."_resized.$extensio");
		$img = imagecreatetruecolor($newwidth, $newheight);
		$img = imagecreatefromstring($image_string);
		$this->image_sizes = getimagesize($file."_resized.$extensio");
		unlink("$file.$extensio");
		unlink("$file"."_resized.$extensio");
		return $img;	
	}
